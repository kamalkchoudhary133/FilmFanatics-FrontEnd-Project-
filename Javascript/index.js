...
function searchInMovies(e) {
    ...
    SEARCH_DEBOUNCE_FLAG = setTimeout(() => {
        ...
        //Searching for a movie with less than 3 characters, causes an error on omdbapi.
        if (trend.length < 3)
            return;

        //For resetting page number on each search
        CURRENT_PAGE = 1;
...
function getMoviesAndParse(trend, page) {
    ...
    getMovies(trend, page)
        .then(({movies = [], totalResults = 0}) => {
            ...
            // Code to scroll to the top after fetch
            window.scrollTo({
                top: 0,
                behavior: "smooth"
            })
...
function showMovieInDetails(movieObj, targetItem) {
    ...
    // Scrolling to target Movie Element
    setTimeout(() => {
        window.scrollTo({
